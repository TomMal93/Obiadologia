...
- Style należące do jednego komponentu POWINNY mieszkać przy komponencie, a nie w `global.css`: dla plików `.astro` używamy lokalnego, zakresowanego bloku `<style>`; dla wyspy React używamy współlokowanego pliku CSS importowanego przez wyspę. Nie dopisujemy jednorazowych, lokalnych reguł do arkusza globalnego.
