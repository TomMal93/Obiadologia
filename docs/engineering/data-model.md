...
- `category` przypisuje składnik do kontrolowanej grupy zakupowej: warzywa i owoce (`produce`), mięso, ryby i wędliny (`meat`), nabiał i jajka (`dairy`), pieczywo i produkty zbożowe (`grains`), produkty spiżarniane (`pantry`) albo przyprawy (`spices`). Strona przepisu prezentuje wyłącznie grupy obecne w danym przepisie, zawsze w tej kolejności.
