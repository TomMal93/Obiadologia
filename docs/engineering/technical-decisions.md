...
| TD-017 | Teksty interfejsu mieszkają w typowanych słownikach `src/i18n/locales/`; polski słownik wyznacza strukturę `AppMessages`, a strony Astro przekazują właściwy słownik do komponentów i wyspy React. Aktywne locale i znaczniki `lang` definiuje `src/i18n/config.ts`; interpolacja oraz odmiana komunikatów nie są składane z fragmentów bezpośrednio w komponentach. | Oddzielenie copy od widoków pozwala redagować treści w jednym miejscu, wymusza kompletność przyszłych słowników przez TypeScript i nie dodaje biblioteki ani wielojęzycznego routingu do MVP. Dane przepisów, hasła indeksu wyszukiwania oraz domenowe formatowanie miar i czasu nie są tekstami UI i wymagają osobnego kontraktu lokalizacji przed uruchomieniem kolejnego języka. |
...
|---|---|---|---|---|
| OPEN-003 | Źródło przepisów: pliki, CMS czy API | model danych, wdrożenia, redakcja | właściciel produktu | na tym etapie źródłem są pliki w repozytorium: katalog trzymamy w Astro Content Collections (jeden plik JSON na przepis w `src/content/recipes/`, walidacja buildowa schematem `Recipe`, dostęp przez `src/data/recipes.ts`). CMS nie jest planowany w tym etapie; wrócić do tematu dopiero przy realnej potrzebie redakcyjnej (konta, workflow autorów) i wtedy porównać CMS/API. `Recipe` pozostaje jednym modelem dla wszystkich ścieżek |
...
| OPEN-008 | Docelowy kontrakt wielu języków: prefiksy i fallback tras, wybór języka, lokalizacja slugów, danych przepisów, wyszukiwania, miar i czasu oraz metadane SEO | publiczne URL-e, model treści, wyszukiwanie, prerendering i SEO | właściciel produktu i osoba techniczna | aktywne jest wyłącznie `pl`; teksty UI korzystają z warstwy przygotowanej w TD-017, ale nie dodawać pustych słowników, przełącznika języka ani tras `/en` i `/de`, dopóki kompletny kontrakt i treści danego języka nie zostaną zaakceptowane |
