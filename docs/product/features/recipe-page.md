...
- Każdą czynność w „Zanim zaczniesz” można lokalnie oznaczyć jako wykonaną. Niewykonana czynność pojawia się również na liście asystenta jako wizualnie lżejszy półkrok z tekstu `stepText`, bez własnego numeru, bezpośrednio przed krokiem `steps` wskazanym przez `beforeStep`. Odhaczenie w panelu albo przy półkroku synchronizuje oba miejsca i ukrywa półkrok; cofnięcie odhaczenia przywraca go. Stan resetuje się po opuszczeniu strony. Bez skryptu panel pozostaje zwykłą listą, a półkroki nie są powielane.
...
| 9 | Przepis z `preparation` pokazuje przed krokami jeden panel „Zanim zaczniesz”, rozdzielający czynności na grupy „Nawet dzień wcześniej” i „Tuż przed lub w trakcie”; pusta grupa jest pomijana. Każda niewykonana czynność jest półkrokiem przed wskazanym `beforeStep`, a jej odhaczenie w dowolnym z dwóch miejsc ukrywa półkrok i synchronizuje stan. Przepis bez `preparation` nie pokazuje panelu ani przełącznika trybu. |
