...
- Każdą czynność w „Zanim zaczniesz” można lokalnie oznaczyć jako wykonaną. Czynność pojawia się również na liście asystenta jako wizualnie lżejszy półkrok z tekstu `stepText`, bez własnego numeru, bezpośrednio przed krokiem `steps` wskazanym przez `beforeStep`. Aktywny półkrok nie pokazuje etykiety stanu „Do zrobienia”. Odhaczenie w panelu albo przy półkroku synchronizuje oba miejsca, ale nie usuwa półkroku: minimalizuje go do zwartego stanu „Gotowe” ze znacznikiem ✓ i jednoliniowym fragmentem treści zapisanym mniejszą czcionką. Odhaczenie właściwego kroku gotowania oznacza również jako wykonane wszystkie półkroki przypisane do tego i wcześniejszych kroków; cofnięcie kroku nie cofa przygotowań. Półkrok nie pokazuje dodatkowego podpisu przywracania, ale ponowne kliknięcie całej karty cofa stan. Stan resetuje się po opuszczeniu strony. Bez skryptu panel pozostaje zwykłą listą, a półkroki nie są powielane.
