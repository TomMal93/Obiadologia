...
Sekcja „Kroki” ma nowszy kształt niż plik referencyjny: zamiast płaskiej listy prowadzi ścieżka z osią etapów rozwijanych po jednym, opisana niżej w „Zachowaniu” i „Prezentacji”. Dla tej sekcji obowiązuje niniejsza specyfikacja, a pozostałe elementy projektu pozostają bez zmian.
...
- Kroki tworzą ścieżkę prowadzoną etap po etapie: pionowa oś z okrągłymi znacznikami łączy karty kolejnych etapów, a nad listą stoi licznik „Krok {bieżący} z {wszystkie}” z poziomym wypełnieniem. Bieżący etap to pierwszy niewykonany; jego karta jest rozwinięta, a pozostałe pozostają zwinięte do jednoliniowej zapowiedzi treści. W danej wersji listy rozwinięty jest najwyżej jeden etap: kliknięcie nagłówka innego etapu zwija poprzedni, a ponowne kliknięcie nagłówka rozwiniętego etapu zwija go bez wybierania kolejnego. Zmiana stanu karty przebiega tym samym ciągłym ruchem wysokości co zwijanie sekcji, a przy `prefers-reduced-motion` następuje natychmiast.
- Rozwinięta karta etapu pokazuje kolejno: przypisane półkroki pod nagłówkiem „Przygotuj przed wykonaniem” wraz z ich licznikiem, a pod linią rozdzielającą numer etapu, etykietę „Finalny krok”, treść kroku i wycentrowaną akcję „Oznacz jako zrobione” z pustym okręgiem. Nagłówek półkroków, ich licznik i etykieta „Finalny krok” należą wyłącznie do etapu z półkrokami.
- Kliknięcie akcji oznacza etap jako wykonany: znacznik na osi zamienia numer na ✓, treść kroku zostaje przekreślona, karta zwija się do zapowiedzi poprzedzonej etykietą „Etap {numer}”, a ścieżka rozwija pierwszy etap wciąż do zrobienia. Ponowne rozwinięcie wykonanego etapu pokazuje akcję w stanie „Zrobione”, której kliknięcie cofa wykonanie i zostawia etap rozwinięty. Krok nie jest usuwany, a stan nie jest zapisywany między wizytami. Bez skryptu strona pokazuje semantyczną listę `ol` z pełną treścią każdego etapu, bez zwijania, akcji i liczników.
...
- Etykiety „Przygotowanie” i „Gotowanie” porządkują oba etapy przepływu. Kroki gotowania mają postać ścieżki: ciągła oś z okrągłymi znacznikami przy lewej krawędzi prowadzi przez wszystkie etapy i nie rozpada się na odcinki między kartami. Karta rozwiniętego etapu jest w tę oś wpięta: jej lewa krawędź siada na linii, znacznik pozostaje na osi i wypada na krawędzi karty, a koralowe obramowanie przejmuje przebieg osi na wysokości karty, więc oś prowadzi do karty i przez nią, zamiast urywać się obok niej. Zwinięte etapy — wykonane i te do zrobienia — mają mniejszy znacznik i stoją zwarcie przy osi. Znacznik etapu do zrobienia ma jasne tło i numer, bieżący — koralowe obramowanie, a wykonany koralowe wypełnienie ze znakiem ✓; odcinek osi prowadzący do bieżącego etapu jest koralowy, dalszy pozostaje jasny. Rozwinięta karta ma delikatnie koralowe tło i koralowe obramowanie, zwinięta zostaje jasna i neutralna. Zwinięty wiersz mieści etykietę „Etap {numer}” (gdy numer ustąpił miejsca znacznikowi ✓), jednoliniową, przyciętą zapowiedź treści i strzałkę stanu. Wewnątrz karty półkroki mają checkboxy i własny nagłówek, a finalny krok numer w koralowym znaczniku, treść i wycentrowaną akcję w kształcie pigułki z pustym okręgiem; pełne koralowe wypełnienie z ✓ komunikuje stan wykonany.
...
| 11 | Składniki i kroki można niezależnie odhaczać. Akcja etapu ma dostępną nazwę zawierającą swój widoczny tekst oraz numer etapu. Wykonany etap pokazuje ✓ na osi, przekreśloną treść i etykietę „Etap {numer}” w zwiniętym wierszu, a po ponownym rozwinięciu pozwala cofnąć wykonanie; wykonany półkrok pokazuje ✓ w checkboxie. Stan ukończenia jest widoczny nie tylko kolorem, nie zmienia danych i resetuje się po opuszczeniu strony. Licznik w wierszu nagłówka „Składniki” podaje aktualny postęp, domyka pierścień i po zebraniu wszystkiego pokazuje komplet. Bez skryptu kontrolki interakcji są ukryte, a obie listy pozostają kompletne i czytelne. |
...
| 19 | Nad krokami stoi licznik „Krok {bieżący} z {wszystkie}”. Rozwinięty jest najwyżej jeden etap danej listy: oznaczenie etapu jako wykonanego zwija jego kartę, przesuwa licznik i rozwija pierwszy etap wciąż do zrobienia, a kliknięcie nagłówka innego etapu rozwija go i zwija poprzedni. |
