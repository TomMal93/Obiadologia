...
Sekcja „Kroki” ma nowszy kształt niż plik referencyjny: zamiast płaskiej listy prowadzi ścieżka z osią niezależnie rozwijanych etapów, opisana niżej w „Zachowaniu” i „Prezentacji”. Dla tej sekcji obowiązuje niniejsza specyfikacja, a pozostałe elementy projektu pozostają bez zmian.

Kompozycję karty etapu porządkuje osobna wizualizacja „Soft dashboard”: jasna karta z ciepłym paskiem nagłówka półkroków, listą półkroków rozdzieloną włosowymi liniami i wpuszczonym koralowym panelem finalnego kroku. Wizualizacja pokazuje przy finalnym kroku także znaczniki czasu i mocy palnika. Model `Recipe` nie ma pól na czas ani temperaturę pojedynczego kroku, więc — zgodnie z regułą powyżej — strona ich nie pokazuje i nie zastępuje ich wartością wyliczoną z treści kroku.
...
- Kroki tworzą ścieżkę prowadzoną etap po etapie: pionowa oś z okrągłymi znacznikami łączy karty kolejnych etapów, a nad listą stoi licznik „Krok {bieżący} z {wszystkie}” z poziomym wypełnieniem. Bieżący etap to pierwszy niewykonany; na wejściu rozwinięta jest tylko jego karta, a pozostałe pozostają zwinięte do jednoliniowej zapowiedzi treści. Karty rozwijają się niezależnie od siebie: kliknięcie nagłówka rozwija albo zwija wyłącznie własną kartę, więc czytelnik może trzymać otwartych kilka etapów naraz. Zmiana stanu karty przebiega tym samym ciągłym ruchem wysokości co zwijanie sekcji, a przy `prefers-reduced-motion` następuje natychmiast.
- Rozwinięta karta etapu pokazuje kolejno: przypisane półkroki wraz z ich licznikiem — ich nagłówek „Przygotuj przed wykonaniem” stoi w wierszu sterowania kartą, na wysokości strzałki zwijania — a pod nimi wyodrębniony panel finalnego kroku z etykietą „Finalny krok”, treścią kroku i pełnoszerokościową akcją „Oznacz jako zrobione”. Nagłówek półkroków, ich licznik i etykieta „Finalny krok” należą wyłącznie do etapu z półkrokami. Numer etapu niesie wyłącznie znacznik osi — karta go nie powtarza: ani widoczną etykietą „Etap {numer}”, ani znacznikiem przy finalnym kroku. Etykieta „Etap {numer}” pozostaje dostępną nazwą kontrolek etapu, niewidoczną na ekranie.
- Kliknięcie akcji oznacza etap jako wykonany: znacznik na osi zamienia numer na ✓, treść kroku zostaje przekreślona, karta zwija się do samej zapowiedzi treści, a ścieżka rozwija pierwszy etap wciąż do zrobienia — nie domykając kart, które czytelnik zostawił otwarte. Ponowne rozwinięcie wykonanego etapu pokazuje akcję w stanie „Zrobione”, której kliknięcie cofa wykonanie i zostawia etap rozwinięty. Krok nie jest usuwany, a stan nie jest zapisywany między wizytami. Bez skryptu strona pokazuje semantyczną listę `ol` z pełną treścią każdego etapu, bez zwijania, akcji i liczników.
...
- Licznik ścieżki nad listą podaje numer bieżącego etapu i ich łączną liczbę, a jego wypełnienie odpowiada liczbie etapów już wykonanych; po wykonaniu wszystkich pokazuje ostatni numer i pełne wypełnienie. Każda wersja listy liczy swój postęp niezależnie. Bez skryptu licznik pozostaje ukryty.
...
- Etykiety „Przygotowanie” i „Gotowanie” porządkują oba etapy przepływu. Kroki gotowania mają postać ścieżki: oś z okrągłymi znacznikami przy lewej krawędzi i karty etapów obok niej. Znacznik etapu do zrobienia ma jasne tło i numer, a bieżący i wykonany koralowe wypełnienie — przy czym wykonany zamienia numer na ✓; odcinek osi prowadzący do bieżącego etapu jest koralowy, dalszy pozostaje jasny, a półkroki rozwiniętego etapu mają na nim własne, drobne znaczniki.
- Karta etapu zostaje jasna także po rozwinięciu — kolor niosą wyłącznie jej wnętrza, a rozwinięcie dokłada koralowe obramowanie i miękki cień. Zwinięty wiersz mieści jednoliniową, przyciętą zapowiedź treści i strzałkę stanu, bez powtórzonego numeru etapu. Rozwinięta karta dzieli się na trzy warstwy: ciepły pasek nagłówka półkroków z ikoną i strzałką zwijania, listę półkroków z checkboxami rozdzieloną włosowymi liniami wraz z licznikiem oraz wpuszczony koralowy panel finalnego kroku. Panel niesie etykietę „Finalny krok” z ikoną, samą treść kroku — bez znacznika z numerem — i pełnoszerokościową akcję: koralowe wypełnienie wzywa do wykonania kroku, a stan wykonany odwraca je w jasną powierzchnię z koralową etykietą i wypełnionym znacznikiem ✓. Etap bez półkroków nie ma paska nagłówka ani wyodrębnionego panelu — jego karta niesie samą treść kroku i tę samą akcję.
...
| 11 | Składniki i kroki można niezależnie odhaczać. Akcja etapu ma dostępną nazwę zawierającą swój widoczny tekst oraz numer etapu. Wykonany etap pokazuje ✓ na osi i przekreśloną treść, a po ponownym rozwinięciu pozwala cofnąć wykonanie; wykonany półkrok pokazuje ✓ w checkboxie. Stan ukończenia jest widoczny nie tylko kolorem, nie zmienia danych i resetuje się po opuszczeniu strony. Licznik w wierszu nagłówka „Składniki” podaje aktualny postęp, domyka pierścień i po zebraniu wszystkiego pokazuje komplet. Bez skryptu kontrolki interakcji są ukryte, a obie listy pozostają kompletne i czytelne. |
...
| 19 | Nad krokami stoi licznik „Krok {bieżący} z {wszystkie}”. Na wejściu rozwinięty jest wyłącznie bieżący etap danej listy, a kliknięcie nagłówka rozwija albo zwija tylko własną kartę, więc otwartych może być kilka etapów naraz. Oznaczenie etapu jako wykonanego zwija jego kartę, przesuwa licznik i rozwija pierwszy etap wciąż do zrobienia, nie ruszając pozostałych otwartych kart. |
