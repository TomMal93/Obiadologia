...
- Kroki obu wersji listy łączy pionowa oś biegnąca przy ich lewej krawędzi. Każdy krok ma na niej własny znacznik połączony łącznikiem z kartą kroku, a odcinek osi prowadzi od tego znacznika do znacznika kroku następnego; ostatni krok zamyka oś na własnym znaczniku. Znacznik pierwszego jeszcze niewykonanego kroku aktywnej listy jest wyróżniony jako miejsce, w którym stoi gotowanie, i przesuwa się wraz z odhaczaniem oraz jego cofaniem. Oś jest dekoracją bez własnej treści: kolejność i stan kroków niosą numeracja listy, teksty kontrolek i liczniki postępu. Bez skryptu oś zaczyna się od kroku pierwszego.
...
- Etykiety „Przygotowanie” i „Gotowanie” porządkują oba etapy przepływu. Wszystkie właściwe kroki, w obu trybach i niezależnie od obecności półkroków, korzystają z tej samej jednolitej, delikatnie koralowej ramki bez pogrubienia którejkolwiek krawędzi. Karty kroków są wsunięte w prawo o kolumnę osi: neutralna, cienka linia z okrągłym znacznikiem przy numerze pierwszego wiersza karty i krótkim łącznikiem do jej krawędzi. Znacznik kroku, przy którym stoi gotowanie, jest koralowym pierścieniem z wypełnionym środkiem, a odcinek osi prowadzący od niego do kolejnego kroku przejmuje koralowy akcent; pozostałe znaczniki są drobnymi, przygaszonymi kropkami. Geometria znacznika nie zmienia się między stanami, więc oś nie przeskakuje, gdy prowadzenie przechodzi do następnego kroku. Półkroki i krok gotowania, do którego prowadzą, tworzą jedną grupę; półkroki odróżniają się lżejszym tłem, checkboxem i subtelnym separatorem. Każdy właściwy krok gotowania ma numer, tekst i wycentrowaną akcję „Oznacz jako zrobione” z pustym okręgiem; pełne koralowe wypełnienie z ✓ komunikuje stan wykonany. W stanie wykonanym treść kroku zostaje przekreślona, a karta zachowuje pełny rozmiar i numer; półkroki nadal minimalizują się do zwartego stanu.
...
| 19 | Kroki obu wersji listy łączy oś: każdy krok ma znacznik przed swoją kartą, na wysokości numeru pierwszego wiersza, a odcinek osi sięga znacznika kroku następnego i kończy się na kroku ostatnim. Wyróżniony znacznik stoi przy pierwszym niewykonanym kroku i przesuwa się przy odhaczaniu oraz cofaniu odhaczenia. Bez skryptu oś zaczyna się od kroku pierwszego, a półkroki zachowują dotychczasową formę wewnątrz wspólnej karty. |
