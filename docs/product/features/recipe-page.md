...
- Każdy krok można lokalnie oznaczyć jako wykonany. Sekcja zapowiada tę interakcję instrukcją, a każdy aktywny krok pokazuje wycentrowany okrągły znacznik z ✓. Po odhaczeniu właściwy krok zachowuje pełny rozmiar i numer, jego treść zostaje przekreślona, a znacznik wypełnia się koralem; ponowne kliknięcie całej karty przywraca stan niewykonany. Krok nie jest usuwany i nie przechodzi automatycznie dalej. Stan nie jest zapisywany między wizytami. Bez skryptu instrukcja i znaczniki akcji pozostają ukryte, a strona pokazuje semantyczną listę `ol` bez kontrolek ukończenia.
...
- Lista w panelu „Zanim zaczniesz” jest początkowo zwinięta i można ją lokalnie rozwinąć oraz ponownie zwinąć. Widoczny opis zachęca do zajrzenia do listy, wskazując, że wcześniejsze przygotowanie może usprawnić późniejsze gotowanie. Kontrolka komunikuje aktualny stan i nie ukrywa nagłówka ani opisu panelu. Bez skryptu kontrolka pozostaje ukryta, a lista jest widoczna.
- Każdą czynność w „Zanim zaczniesz” można lokalnie oznaczyć jako wykonaną. Czynność pojawia się również na liście asystenta jako wizualnie lżejszy półkrok z tekstu `stepText`, bez własnego numeru, wewnątrz wspólnej powierzchni z krokiem `steps` wskazanym przez `beforeStep`. Półkroki poprzedzają ten krok, a wspólna ramka i ciągły akcent wizualny pokazują, do którego kroku prowadzą. Aktywny półkrok ma wycentrowany pionowo, pusty kwadratowy checkbox bez dodatkowej akcji tekstowej; po oznaczeniu checkbox pokazuje ✓, a półkrok minimalizuje się do zwartego stanu „Gotowe” z jednoliniowym fragmentem treści zapisanym mniejszą czcionką. Odhaczenie w panelu albo przy półkroku synchronizuje oba miejsca i pozostawia możliwość cofnięcia. Odhaczenie właściwego kroku gotowania oznacza również jako wykonane wszystkie półkroki przypisane do tego i wcześniejszych kroków; cofnięcie kroku cofa także stan tych półkroków. Ponowne kliknięcie całej karty półkroku cofa jego stan. Stan resetuje się po opuszczeniu strony. Bez skryptu panel pozostaje zwykłą listą, a półkroki nie są powielane.
...
- Etykiety „Przygotowanie” i „Gotowanie” porządkują oba etapy przepływu. Wszystkie właściwe kroki, w obu trybach i niezależnie od obecności półkroków, korzystają z tej samej delikatnie koralowej ramki oraz koralowego akcentu przy lewej krawędzi. Półkroki i krok gotowania, do którego prowadzą, tworzą jedną grupę; półkroki odróżniają się lżejszym tłem, checkboxem i subtelnym separatorem. Każdy właściwy krok gotowania ma numer, tekst i wycentrowany okrągły znacznik akcji z ✓, używający jasnokoralowego tła, ciemnego koralu oraz koralowego obramowania. W stanie wykonanym znacznik wypełnia się koralem, treść kroku zostaje przekreślona, a karta zachowuje pełny rozmiar i numer; półkroki nadal minimalizują się do zwartego stanu.
...
| 9 | Przepis z `preparation` pokazuje przed krokami jeden panel „Zanim zaczniesz”, rozdzielający czynności na grupy „Nawet dzień wcześniej” i „Tuż przed lub w trakcie”; pusta grupa jest pomijana. Lista jest początkowo zwinięta, opis wskazuje korzyść z jej rozwinięcia, można ją rozwinąć i ponownie zwinąć, a bez skryptu pozostaje widoczna. Każda czynność jest półkrokiem bez etykiety „Do zrobienia” przed wskazanym `beforeStep`, a jej odhaczenie w dowolnym z dwóch miejsc synchronizuje stan bez usuwania półkroku i pozostawia możliwość cofnięcia. Odhaczenie kroku gotowania oznacza jako wykonane półkroki przypisane do niego i wcześniejszych kroków, a cofnięcie kroku cofa również ich stan. Przepis bez `preparation` nie pokazuje panelu ani przełącznika trybu. |
...
| 11 | Składniki i kroki można niezależnie odhaczać; klikalność kroków jest jawnie opisana nad listą i przez dostępne nazwy kontrolek. Wykonany właściwy krok zachowuje pełny rozmiar i numer, ma przekreśloną treść oraz wypełniony znacznik ✓; wykonany półkrok minimalizuje się bez przekreślania. Oba pozostawiają możliwość przywrócenia. Stan ukończenia jest widoczny nie tylko kolorem, nie zmienia danych i resetuje się po opuszczeniu strony. Licznik w wierszu nagłówka „Składniki” podaje aktualny postęp, domyka pierścień i po zebraniu wszystkiego pokazuje komplet. Bez skryptu instrukcje interakcji są ukryte, a obie listy pozostają kompletne i czytelne. |
