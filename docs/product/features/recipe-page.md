...
- Przepis z etapami wspierającymi ma dwie wersje kroków i każdy tryb pokazuje własną. „Tryb asystenta” pokazuje `steps` — kroki właściwego gotowania, obok widocznych sekcji „Wcześniej” i „Przygotowanie”. „Tylko kroki” pokazuje `stepsOnly` — wersję samodzielną, która niesie także czynności ukrytych sekcji (krojenie, namoczenie, sprzęt), więc jest innym tekstem i może mieć inną liczbę kroków. Obie wersje mają wspólny nagłówek „Kroki”, a widoczna jest zawsze dokładnie jedna. Bez skryptu strona pokazuje etapy wspierające razem z wersją asystenta, a wersja samodzielna pozostaje ukryta, aby ta sama instrukcja nie pojawiła się dwa razy.
...
- Strona nie planuje godzin gotowania. Sekcja „Wcześniej” podaje wyłącznie wymagane wyprzedzenie względem podania („na 2 godz przed podaniem”); użytkownik nie podaje pory posiłku i strona nie wylicza godzin startu.
- Gdy przepis ma choć jedną z sekcji `advance`/`preparation`, strona wymaga wybrania „Tryb asystenta” albo „Tylko kroki” przed pokazaniem dalszej części przepisu. Początkowo żadna opcja nie jest aktywna, a etapy wspierające, kroki, porady i notatka prototypowa są ukryte. „Tryb asystenta” pokazuje pełne prowadzenie, kroki w wersji `steps` i dalszą treść, natomiast „Tylko kroki” pomija etapy wspierające i pokazuje samodzielną wersję kroków (`stepsOnly`) wraz z dalszą treścią. Wybór jest lokalny i resetuje się po opuszczeniu strony. Mechanizm jest wzbogaceniem progresywnym — bez skryptu wybór pozostaje ukryty, a strona pokazuje pełną treść. Przepis bez pól `advance`/`preparation` nie pokazuje wyboru i od razu prezentuje kroki.
...
- Sekcje „Wcześniej”, „Przygotowanie” i „Kroki” korzystają ze wspólnej neutralnej, jasnej ramki.
...
| 10 | Przed wyborem trybu żadna dalsza część przepisu nie jest widoczna i żadna opcja nie jest aktywna. Po wybraniu „Tryb asystenta” widoczne są etapy wspierające, a kroki są wersją `steps`; „Tylko kroki” ukrywa etapy wspierające i pokazuje samodzielną wersję kroków (`stepsOnly`), zawierającą czynności z ukrytych sekcji. Strona nigdzie nie pyta o porę posiłku ani nie wylicza godzin startu. Bez skryptu wybór pozostaje ukryty, a treść jest pełna. |
