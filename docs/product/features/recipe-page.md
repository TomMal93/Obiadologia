...
- Przepis z etapami wspierającymi ma dwie wersje kroków i każdy tryb pokazuje własną. „Rozłóż w czasie” pokazuje `steps` — kroki właściwego gotowania, obok widocznych sekcji „Wcześniej” i „Przygotowanie”. „Tylko kroki” pokazuje `stepsOnly` — wersję samodzielną, która niesie także czynności ukrytych sekcji (krojenie, namoczenie, sprzęt), więc jest innym tekstem i może mieć inną liczbę kroków. Obie wersje mają wspólny nagłówek „Kroki”, a widoczna jest zawsze dokładnie jedna. Bez skryptu strona pokazuje etapy wspierające razem z wersją „Rozłóż w czasie”, a wersja samodzielna pozostaje ukryta, aby ta sama instrukcja nie pojawiła się dwa razy.
...
- Tryb „Rozłóż w czasie” zaczyna się panelem „Kiedy zacząć”. Po podaniu pory podania pokazuje rozpoczęcie głównego gotowania (`pora − preparationMinutes`), a przy każdej czynności „Wcześniej” godzinę jej rozpoczęcia (`pora − leadTimeMinutes`); start przypadający poprzedniego dnia jest jednoznacznie oznaczony. Pomocnik jest wzbogaceniem progresywnym — bez skryptu pozostaje ukryty, a sekcja i tak pokazuje wymagane wyprzedzenie.
- Gdy przepis ma choć jedną z sekcji `advance`/`preparation`, strona wymaga wybrania „Rozłóż w czasie” albo „Tylko kroki” przed pokazaniem dalszej części przepisu. Początkowo żadna opcja nie jest aktywna, a etapy wspierające, kroki, porady i notatka prototypowa są ukryte. „Rozłóż w czasie” pokazuje pełne prowadzenie, kroki w wersji `steps` i dalszą treść, natomiast „Tylko kroki” pomija etapy wspierające i pokazuje samodzielną wersję kroków (`stepsOnly`) wraz z dalszą treścią. Wybór jest lokalny i resetuje się po opuszczeniu strony. Mechanizm jest wzbogaceniem progresywnym — bez skryptu wybór pozostaje ukryty, a strona pokazuje pełną treść. Przepis bez pól `advance`/`preparation` nie pokazuje wyboru i od razu prezentuje kroki.
...
| 10 | Przed wyborem trybu żadna dalsza część przepisu nie jest widoczna i żadna opcja nie jest aktywna. Po wybraniu „Rozłóż w czasie” pomocnik „Kiedy zacząć” wylicza początek głównego gotowania i godzinę rozpoczęcia każdej czynności z wyprzedzeniem, a kroki są wersją `steps`; „Tylko kroki” ukrywa etapy wspierające i pokazuje samodzielną wersję kroków (`stepsOnly`), zawierającą czynności z ukrytych sekcji. Bez skryptu wybór i pomocnik pozostają ukryte, a treść jest pełna. |
...
| 16 | W przepisie z etapami wspierającymi „Rozłóż w czasie” i „Tylko kroki” pokazują różne teksty kroków: wersja samodzielna niesie czynności ukrytych sekcji „Wcześniej” i „Przygotowanie”, a widoczna jest zawsze dokładnie jedna lista, także bez skryptu. Numeracja każdej wersji zaczyna się od jedynki i wraca po cofnięciu odhaczenia. |
