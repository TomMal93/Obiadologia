...
- jedna przewijana kolumna treści: opis, Składniki, przełącznik trybu gotowania, „Zanim zaczniesz” i Kroki;
...
- Strona prezentuje w kolejności: hero ze zdjęciem albo placeholderem, tytułem (`h1`) i tagami; pasek trudności, porcji i czasu przygotowania; opis; listę składników z nagłówkiem „Składniki”; opcjonalną sekcję „Zanim zaczniesz”; kroki przygotowania z nagłówkiem „Kroki”; opcjonalną sekcję porad „Coś jeszcze”.
...
- Strona pokazuje kroki przygotowania jako uporządkowaną, numerowaną listę (`ol`) w kolejności zapisanej w danych ([data-model.md](../../engineering/data-model.md)). Przepis bez sekcji „Zanim zaczniesz” ma jedną listę z pola `steps`.
- Przepis z sekcją „Zanim zaczniesz” ma dwie wersje kroków i każdy tryb pokazuje własną. „Tryb asystenta” pokazuje `steps` — kroki właściwego gotowania, obok widocznej sekcji „Zanim zaczniesz”. „Tylko kroki” pokazuje `stepsOnly` — wersję samodzielną, która niesie także czynności ukrytej sekcji (krojenie, odsączenie, sprzęt), więc jest innym tekstem i może mieć inną liczbę kroków. Obie wersje mają wspólny nagłówek „Kroki”, a widoczna jest zawsze dokładnie jedna. Bez skryptu strona pokazuje sekcję „Zanim zaczniesz” razem z wersją asystenta, a wersja samodzielna pozostaje ukryta, aby ta sama instrukcja nie pojawiła się dwa razy.
...
- Gdy przepis ma pole `preparation`, strona pokazuje przed krokami jedną sekcję „Zanim zaczniesz” z czynnościami wykonywanymi przed właściwym gotowaniem: mise en place, kompletowaniem sprzętu i tym, co da się zrobić z wyprzedzeniem.
- Sekcja nie dzieli czynności według wyprzedzenia i nie podaje żadnych minut ani godzin. Informacja „możesz to zrobić już wieczorem” jest częścią zdania czynności, zapisaną redakcyjnie w danych ([data-model.md](../../engineering/data-model.md)). Strona nie pyta o porę posiłku i niczego nie planuje w czasie.
- Gdy przepis ma pole `preparation`, strona wymaga wybrania „Tryb asystenta” albo „Tylko kroki” przed pokazaniem dalszej części przepisu. Początkowo żadna opcja nie jest aktywna, a sekcja „Zanim zaczniesz”, kroki, porady i notatka prototypowa są ukryte. „Tryb asystenta” pokazuje pełne prowadzenie, kroki w wersji `steps` i dalszą treść, natomiast „Tylko kroki” pomija sekcję „Zanim zaczniesz” i pokazuje samodzielną wersję kroków (`stepsOnly`) wraz z dalszą treścią. Wybór jest lokalny i resetuje się po opuszczeniu strony. Mechanizm jest wzbogaceniem progresywnym — bez skryptu wybór pozostaje ukryty, a strona pokazuje pełną treść. Przepis bez pola `preparation` nie pokazuje wyboru i od razu prezentuje kroki.
...
| 9 | Przepis z polem `preparation` pokazuje sekcję „Zanim zaczniesz” przed krokami; przepis bez tego pola jej nie pokazuje i nie udostępnia przełącznika trybu. Sekcja nie zawiera minut ani godzin. |
| 10 | Przed wyborem trybu żadna dalsza część przepisu nie jest widoczna i żadna opcja nie jest aktywna. Po wybraniu „Tryb asystenta” widoczna jest sekcja „Zanim zaczniesz”, a kroki są wersją `steps`; „Tylko kroki” ukrywa tę sekcję i pokazuje samodzielną wersję kroków (`stepsOnly`), zawierającą jej czynności. Strona nigdzie nie pyta o porę posiłku ani nie wylicza godzin startu. Bez skryptu wybór pozostaje ukryty, a treść jest pełna. |
...
| 16 | W przepisie z sekcją „Zanim zaczniesz” „Tryb asystenta” i „Tylko kroki” pokazują różne teksty kroków: wersja samodzielna niesie czynności ukrytej sekcji, a widoczna jest zawsze dokładnie jedna lista, także bez skryptu. Numeracja każdej wersji zaczyna się od jedynki i wraca po cofnięciu odhaczenia. |
