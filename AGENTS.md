...
| ścieżka kroków gotowania (oś etapów, półkroki, akcja „Oznacz jako zrobione”) | `src/components/RecipeStepJourney.astro` |
| style współlokowane ścieżki kroków | `src/components/RecipeStepJourney.css` |
...
| Strona przepisu `/recipes/:slug` | wersja wstępna | implementacja prezentuje pola modelu `Recipe` (zdjęcie/placeholder, opis, szacunkowe wartości odżywcze na porcję, trudność, czas, porcje, tagi, składniki z mieszaną formą miary: metryczną, domową albo obiema po ukośniku, zgodnie z polem `measure` składnika — bez przełącznika jednostek); zmiana liczby porcji skaluje ilości; opcjonalny, klikalny panel „Zanim zaczniesz” grupuje przygotowania możliwe dzień wcześniej oraz wykonywane tuż przed gotowaniem lub w trakcie; kroki mają postać ścieżki (`src/components/RecipeStepJourney.astro`): oś z numerowanymi znacznikami, licznik „Krok N z M” i niezależnie rozwijane karty etapów (na wejściu otwarty jest sam bieżący etap) — rozwinięty etap pokazuje przypisane półkroki z ich licznikiem oraz finalny krok z akcją „Oznacz jako zrobione”, a jego wykonanie zwija kartę i rozwija kolejny etap do zrobienia; odhaczenie kroku domyka też przypisane do niego półkroki i jest synchroniczne z panelem; nagłówki „Zanim zaczniesz” i „Kroki” są prezentowane bez dodatkowych etykiet etapów; panel współpracuje z przełącznikiem „Tryb asystenta / Tylko kroki” jako wzbogacenie progresywne; `stepsOnly` pozostaje samodzielną wersją dla trybu bez panelu; opcjonalne `tips` tworzy panel „Coś jeszcze”; opublikowane rekordy otrzymują prerenderowane trasy |
...
7. Podczas iteracji, pracy nad kodem i przed finalizacją zwykłego zadania bez commita uruchamiaj szybką pętlę weryfikacyjną (`pnpm test`, `pnpm typecheck`, `pnpm lint`). Pełną bramkę (`pnpm verify`, w tym `pnpm test:e2e`) uruchamiaj wyłącznie przed utworzeniem commita albo na wyraźne polecenie użytkownika. W razie wystąpienia błędu na pełnej bramce przerwij procedurę commitowania, przedstaw raport błędu i poczekaj na decyzję użytkownika. Jeżeli użytkownik wprost zażąda commita bez testów (np. poleceniem typu „git commit bez testów”), pomiń testy/bramkę i wykonaj wyłącznie sam commit. Szczegółowy workflow opisuje `docs/engineering/quality-requirements.md`.
